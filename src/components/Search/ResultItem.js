...
function ResultItem(props){
  ...
  return (
    <div className="result_item">
      ...
      {type !== 'person' ? (
        <div className="result_item_image">{poster_path ? (<img src={`https://image.tmdb.org/t/p/w300${poster_path}`} alt="poster" />) : (<i className="fas fa-film"></i>) }</div>
      ) : (
        <div className="result_item_image">{profile_path ? (<img src={`https://image.tmdb.org/t/p/w300${profile_path}`} alt="person" />): (<i className="fas fa-user"></i>)}</div>
      )}

      <div className="result_item_content">
