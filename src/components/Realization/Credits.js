...
function Credits(props){
  ...
  React.useEffect(() => {
    setCastVisible(Math.min((window.innerWidth > 576 ? 9 : 5), props.cast.length));
    setCrewVisible(Math.min((window.innerWidth > 576 ? 9 : 5), props.crew.length));
  }, []);
        ...
        <React.Fragment>
          ...
          <section className="realization_credits_cast">
            {cast.slice(0, castVisible).map(person => (
              <section key={`${person.id}-${person.character}`} className="cast_item">
                <section className="cast_item_image">
                  {person.profile_path === null ? <i className="far fa-user"></i> : <img src={`https://image.tmdb.org/t/p/w45${person.profile_path}`} />}
                </section>
        ...
        <React.Fragment>
          ...
          <section className="realization_credits_crew">
            {crew.slice(0, crewVisible).map(person => (
              <section key={`${person.id}-${person.job}`} className="crew_item">
